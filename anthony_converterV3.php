...
else
{
	...
	for ($a = 0; $a < count($lotNumberArray); $a++)
	{
		...
		$pdf->Cell(1,5,"",0,0);
		$pdf->Cell(130,5,"CORRECTIVE ACTION REPORT",0,0);
		$pdf->SetFont('SJIS', '', 9);
		$pdf->Cell(16,4,"",0,0);
		...
		else
		{
			...
			$sql = "SELECT employeeId FROM hr_employee WHERE idNumber LIKE '".$userName."' ";	$getName = $db->query($sql);
			if($getName->num_rows > 0)	{	$getNameResult = $getName->fetch_array(); $signatureId = $getNameResult['employeeId'];	}
		$pdf->Image('../11-A Employee List V2/esignatures/'.$signatureId.'.jpg', ($pdf->GetX()+(47.4-20)), $pdf->GetY(), 10, 5);
		...
		//$pdf->Cell(47.4,4.5,"Ariel Yason",'TRB',0,'L'); //comment 2018-09-06 by nabel -rose-
		$pdf->Cell(47.4,4.5,"Roldan Macalindro",'TRB',0,'L');
		$pdf->Ln();
		...
		$pdf->MultiCell(50.7,4.5,$sql5Result['cparDispositionDetails'],'LR');
		...
		//----------------- PROCESS CAUSE ---------------------
		$pdf->SetFont('SJIS','I');
		$pdf->Cell(95,4.5,"PROCESS CAUSE:",'LR',0,'L');
		$pdf->Cell(95,4.5,"FLOW-OUT CAUSE:",'LR',0,'L');
		$pdf->SetFont('SJIS','');
		$pdf->Ln();
		...
		$currentY9 = $pdf->GetY();
		$currentY10 = $pdf->GetY();
		$pdf->MultiCell(95,3,characterReplace($sql5Result['cparCauseProcess']),'LR');
		$pdf->SetXY($pdf->GetX()+95,$currentY9);
		$pdf->MultiCell(95,3,characterReplace($sql5Result['cparCauseFlowOut']),'LR');
		$pdf->SetXY($pdf->GetX()+95,$currentY10);
		$pdf->Ln();

		//----------------- PROCESS CAUSE BLANK 2 ---------------------
		$pdf->Cell(95,4.5,"",'LR',0,'L');
		$pdf->Cell(95,4.5,"",'LR',0,'L');
		$pdf->Ln();
		...
		//----------------- PROCESS ---------------------
		$pdf->SetFont('SJIS','I');
		$pdf->Cell(95,4.5,"PROCESS:",'LR',0,'L');
		$pdf->Cell(95,4.5,"FLOW-OUT:",'LR',0,'L');
		$pdf->SetFont('SJIS','');
		$pdf->Ln();
		//~ $sql5Result['cparCorrectiveProcess'] = "";
		//----------------- PROCESS BLANK 1 ---------------------
		$currentY11 = $pdf->GetY();
		$currentY12 = $pdf->GetY();
		if($cparId == '26745')
		...
		$pdf->MultiCell(95,4.5,characterReplace($sql5Result['cparCorrectiveProcess']),'LR');
		$pdf->SetXY($pdf->GetX()+95,$currentY11);
		$pdf->MultiCell(95,4.5,characterReplace($sql5Result['cparCorrectiveFlowOut']),'LR');
		$pdf->SetXY($pdf->GetX()+95,$currentY12);
		$pdf->Ln();
		
		$plusHeight = 4;
		
		//~ $height = 4.5 + $plusHeight;
		$height = 13;
		
		//----------------- PROCESS BLANK 2 ---------------------
		$pdf->Cell(95,$height,"",'LR',0,'L');
		$pdf->Cell(95,$height,"",'LR',0,'L');
		$pdf->Ln();
		...
		$pdf->Cell(39.5,4,$sql5Result['cparCorrectiveProcessIncharge'],'B',0,'L');
		$pdf->SetFont('SJIS','',7);
		...
		$pdf->Cell(20,4,$outputDate,'B',0,'L');
		$pdf->Cell(18.8,4.5,"  Incharge:",0,0,'L');
		$pdf->Cell(26.5,4,$sql5Result['cparCorrectiveFlowOutIncharge'],'B',0,'L');
...
//~ 
		...
		$pdf->SetFont('SJIS','');
		$pdf->SetFont('SJIS','I');
		$pdf->Cell(57,4.5,"(1 month monitoring)",'TR',0,'L');
		$pdf->SetFont('SJIS','');
		...
		// ---------------------------- Create Box Status -----------------------
		$pdf->SetXY(173,237.5+$plusHeight);
		$pdf->Cell(4,3,"",1,0);
		
		$pdf->SetXY(173,242+$plusHeight);
		$pdf->Cell(4,3,"",1,0);
		
		$pdf->SetXY(173,247+$plusHeight);
		$pdf->Cell(4,3,"",1,0);
