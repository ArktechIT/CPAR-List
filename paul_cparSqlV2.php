<?php
	...
	if($_POST['Submit']=='Submit') //1;
	{
		...
		//~ {
			//~ echo $_POST['assignEmployee'];
			//~ echo "<br>".$_POST['inputName1'];
			//~ exit(0);
		//~ }
		...
		if($queryUp->num_rows > 0)
		{
			...
			if($_SESSION['idNumber']=='0407') //mas bago pa sa bago;
			{
			...
			else
			{
				...
				if($queryMaxId AND $queryMaxId->num_rows > 0)
				{
					...
					if($querySection->num_rows > 0)
					{
						...
						$_POST['inputSection'] = $resultSection['cparSection'];
	...
	if($_POST['Submit']=='Confirm') //3
	{
		...
		if($queryMaxID->num_rows > 0)
		{
			...
			$sql = "UPDATE qc_cpar SET checkedBy = '".$_SESSION['idNumber']."', status = 4 WHERE cparId LIKE '".$cparId."' LIMIT 1";
			$queryUpdate1 = $db->query($sql);
			header("location: ../dashboard.php");				
		}
	}
	
	if($_POST['Submit']=='Verify') //4;
	{
		$cparId = $_POST['cparId'];
		...
		if($queryMaxID AND $queryMaxID->num_rows > 0)
		{
			...
			else
			{
				$sql = "INSERT INTO system_notification
								(notificationId, notificationTarget, targetType) 
						VALUES
								(".$maxNotificationId.", '0407', 2)
				";					
			}
				$queryInsert1 = $db->query($sql);

			$sql = "UPDATE qc_cpar SET verifyBy = '".$_SESSION['idNumber']."', status = 5 WHERE cparId LIKE '".$cparId."' LIMIT 1";
			$queryUpdate1 = $db->query($sql);
			header("location: ../dashboard.php");	
		}
	}
	
	if($_POST['Submit']=='Approved') //5;
	{
		$cparId = $_POST['cparId'];
		$notificationIdArray = array();
		$sql = "SELECT notificationId FROM system_notificationdetails WHERE notificationKey LIKE '".$cparId."'";
	...
	if($_POST['Disapproved']=='Disapproved') //sir ariel
	{
		...
		if($query->num_rows > 0)
		{
			...
			if($queryMaxID->num_rows > 0)
			{
				...
				$sql = "UPDATE qc_cpar SET verifyBy = '', status = 4 WHERE cparId LIKE '".$cparId."' LIMIT 1";
	...
	if($_POST['Assign']=='SUBMIT') //sir ryan;
	{
		...
		if($query AND $query->num_rows > 0)
		{
			...
			if($queryMaxID->num_rows > 0)
			{
				...
				$sql = "UPDATE qc_cpar SET assignEmployee = '".$assignEmployee."', cparSourcePerson = '".$cparSourcePerson."', status = 1 WHERE cparId LIKE '".$cparId."' LIMIT 1";
				$queryUpdate2 = $db->query($sql);
				header("location: ../dashboard.php");
			}
		}
